...
def buildDepartmentJobs(dictFile):
    ...
    if not (not resultEmployees):
        ...
        for employee in resultEmployees:
            ...
            hireDate = employee[5] #PROBLEMA: Python não permite serialização de dateTimes! Solução: Fica por default em str....not sure se isto vai trazer problemas nas queries
                                    # Existe uma segunda solução (utilização de default o json_util) mas o JSON fica estranho...rever melhor se é possivel fazer essa alteração
                                    # https://stackoverflow.com/questions/11875770/how-to-overcome-datetime-datetime-not-json-serializable/36142844#36142844 -> SOLUÇÕES 
            jobId = employee[6]
            salary = employee[7]
    ...
    if not (not resultHistoric):
        ...
        for historic in resultHistoric:
            ...
            if employeeId not in firstElems:
                ...
                #PROBLEMA: Para sabermos qual o trabalho atual do employee do histórico este vai ser pesquisado utilizando a informação que se apresenta 
                # ou de outra forma (ID, exemplo)? Será que n vai haver uma pessoa com as msm caracteristicas so que com IDs diferentes? 
                # Nesse caso a pesquisa n devolve a pessoa correta...CASO RARO! 
...
try:
    ...
    for department in result:
        ...
        mydb = myclient["HR"]        # Base de Dados : HR
        mycol = mydb["departments"] # Collection : departaments
        x = mycol.insert_one(dictFile)
